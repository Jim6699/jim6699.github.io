const questions = [
    {
        question: "1. 您更符合哪一项？",
        answers: [
            { text: "冒险精神—对新事物下决心去做", value: "D" },
            { text: "适应程度—轻松自如融入任何环境", value: "S" },
            { text: "生动表现—表情生动多手势", value: "I" },
            { text: "分析能力—准确知道所有细节之间的逻辑关系", value: "C" }
        ]
    },
    {
        question: "2. 您更符合哪一项？",
...
function renderQuestion(index) {
    ...
    container.innerHTML = `
        ...
        ${question.answers.map(answer => `
            <div class="answer" onclick="selectAnswer('${answer.value}')">${answer.text}</div>
        `).join('')}
    `;
    document.getElementById('prev-btn').style.display = index === 0 ? 'none' : 'inline';
    document.getElementById('next-btn').style.display = index === questions.length - 1 ? 'none' : 'inline';
}
